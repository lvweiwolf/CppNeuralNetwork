...
#define MATRIX_MATRIX_OP_IMPL(op)                                       \
    ...
    {                                                                   \
        ...
        {                                                               \
            ...
            throw std::runtime_error("matrix incompatible operation."); \
                                                                        \
        return *this;                                                   \
    }
...
#define GLOBAL_MATRIX_SCALAR_OP(op)              \
    template <typename T>                        \
    Matrix<T>                                    \
    operator op(const Matrix<T> &m, const T val) \
    {                                            \
        Matrix<T> mat(m._row, m._col);           \
        auto n_elem = m._row * m._col;           \
        for (auto i = 0; i < n_elem; ++i)        \
            mat._data[i] = m._data[i] op val;    \
                                                 \
        return mat;                              \
    }

    GLOBAL_MATRIX_SCALAR_OP(+)
    GLOBAL_MATRIX_SCALAR_OP(-)
    GLOBAL_MATRIX_SCALAR_OP(*)
    GLOBAL_MATRIX_SCALAR_OP(/)

#define GLOBAL_MATRIX_MATRIX_OP(op)                                     \
    template <typename T>                                               \
    Matrix<T>                                                           \
    operator op(const Matrix<T> &lhs, const Matrix<T> &rhs)             \
    {                                                                   \
        if (lhs._row == rhs._row && lhs._col == rhs._col)               \
        {                                                               \
            Matrix<T> mat(lhs._row, lhs._col);                          \
            auto n_elem = lhs._row * lhs._col;                          \
            for (auto i = 0; i < n_elem; ++i)                           \
                mat._data[i] = lhs._data[i] op rhs._data[i];            \
                                                                        \
            return mat;                                                 \
        }                                                               \
        else                                                            \
    ...
    operator*(const Matrix<T> &lhs, const Matrix<T> &rhs)
    {
        ...
        {
            auto &A = lhs._data; // M x K
            auto &B = rhs._data; // K x N
            auto &M = lhs._row;
            auto &K = lhs._col;
            auto &N = rhs._col;
            auto &LDA = K;
